...
func (r *AudioRepository) CreateAudioFile(audioFile *models.AudioFile) error {
	query := `
		INSERT INTO audio_files (id, meeting_id, filename, file_path, file_size, 
			duration, format, uploaded_at, processed)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
	`
	_, err := r.db.Exec(query, audioFile.ID, audioFile.MeetingID, audioFile.Filename,
...
func (r *AudioRepository) GetAudioFilesByMeetingID(meetingID uuid.UUID) ([]models.AudioFile, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&af.ID, &af.MeetingID, &af.Filename, &af.FilePath,
